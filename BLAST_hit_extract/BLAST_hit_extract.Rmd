...
blastn_buoyancy_assemblies_best_hits <- blastn_buoyancy_assemblies_id_overlap %>%
  ...
  group_by(qseqid_overlap) %>%
  arrange(evalue, desc(bitscore)) %>%
  slice_min(evalue, n = 1, with_ties = FALSE) %>% # Keep only the hit with the lowest evalue, highest biscore (if evalue tied)
  select(qseqid_overlap, qseqid, overlap_group, sseqid, pident, length, gene_target, everything()) %>%
  ungroup()
