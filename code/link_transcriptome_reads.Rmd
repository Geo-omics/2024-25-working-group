...
metadata_from_paper <- readxl::read_excel("data/metadata/zepernick_MRA_table_diel_Erie_metaT.xlsx") %>% 
  janitor::clean_names() %>% 
...
data_for_deseq <-   DESeqDataSetFromMatrix(count_featurecounts[,samples],colData = metadata_for_deseq2[samples,],design = ~day_or_night)
...
enrichment <- gage(deseq_w_kegg$log2FoldChange %>%  `names<-`(deseq_w_kegg$xref_id), gsets = kegg_def$kg.sets, ref = NULL, samp = NULL,species = "ko")
...
enriched_df %>% 
  group_by(pathway) %>% 
  slice_min(order_by = p.val, n = 1) %>% 
  mutate(sig = p.val < 0.05) %>% 
